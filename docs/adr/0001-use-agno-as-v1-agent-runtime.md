...
- Mininet maps `<experiment-run-id>:<agent-id>` to Agno's session identity and the Mininet invocation ID to the Agno run identity. Learned memory uses no user identity unless explicitly declared; run scope uses the session identity, while agent scope uses the compiled agent ID across runs.
- Agno state is persisted in a caller-selected private SQLite database. The immutable audit/ledger record captures the Agno version, effective memory settings, and session identities rather than treating the mutable Agno database as experimental evidence.
- Shared operational state uses a separate Mininet-owned transactional store. Agents receive only declared run/deployment scopes and propose versioned updates in their structured response; Mininet authorizes and records those updates before executing network proposals.
